#!/bin/zsh
#
# notarize.sh: Sign dist/GlobalProtect.app with a Developer ID certificate,
# notarize it with Apple, staple the ticket, and produce a distributable zip.
#
# One-time setup:
#   xcrun notarytool store-credentials globalprotect-notary \
#       --key AuthKey_XXXX.p8 --key-id XXXX --issuer <issuer-uuid>
#
# Usage: zsh GlobalProtectApp/Scripts/notarize.sh
#   GP_SIGN_IDENTITY="Developer ID Application: Name (TEAMID)"  (required)
#   GP_NOTARY_PROFILE=globalprotect-notary                       (default)
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
APP_DIR="${REPO_ROOT}/GlobalProtectApp"
APP="${REPO_ROOT}/dist/GlobalProtect.app"
ENTITLEMENTS="${APP_DIR}/Support/GlobalProtect.entitlements"
IDENTITY="${GP_SIGN_IDENTITY:-}"
NOTARY_PROFILE="${GP_NOTARY_PROFILE:-globalprotect-notary}"

[[ -d "$APP" ]] || { echo "error: ${APP} missing; run Scripts/bundle.sh first" >&2; exit 1; }
[[ -n "$IDENTITY" ]] || { echo "error: set GP_SIGN_IDENTITY to your Developer ID Application identity" >&2; exit 1; }
...
ZIP="${REPO_ROOT}/dist/GlobalProtect-${VERSION}.zip"
