...
# dist/GlobalProtect.app, embedding gpclient, its Homebrew dylibs, and
# OpenConnect's vpnc-script so the bundle is self-contained.
#
# Usage: zsh GlobalProtectApp/Scripts/bundle.sh [--skip-gpclient]   (--skip-gpclient: do not embed gpclient; rely on Homebrew or a custom path)
...
APP_DIR="${REPO_ROOT}/GlobalProtectApp"
DIST_DIR="${REPO_ROOT}/dist"
APP_BUNDLE="${DIST_DIR}/GlobalProtect.app"
...
echo "==> Building GlobalProtect (release)"
xcrun swift build -c release --package-path "$APP_DIR" --product GlobalProtect
...
cp "${BIN_PATH}/GlobalProtect" "${MACOS_DIR}/GlobalProtect"
chmod +x "${MACOS_DIR}/GlobalProtect"

# SwiftPM resource bundle (icon etc.) plus a flat copy of the icon for CFBundleIconFile.
if [[ -d "${BIN_PATH}/GlobalProtect_GlobalProtect.bundle" ]]; then
    ditto "${BIN_PATH}/GlobalProtect_GlobalProtect.bundle" "${RESOURCES_DIR}/GlobalProtect_GlobalProtect.bundle"
fi
cp "${APP_DIR}/Sources/GlobalProtect/Resources/AppIcon.icns" "${RESOURCES_DIR}/AppIcon.icns"
...
codesign --force --deep --sign - --entitlements "${APP_DIR}/Support/GlobalProtect.entitlements" "$APP_BUNDLE" >/dev/null
