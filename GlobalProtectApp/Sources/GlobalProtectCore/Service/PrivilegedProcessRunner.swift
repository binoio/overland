...
public actor PrivilegedProcessRunner: ProcessRunning {
    ...
    #!/bin/bash
    # GlobalProtect for macOS: root-side launcher.
    # Usage: gp-privileged-wrapper.sh <session-dir> <executable> [args…]
    #
    ...
    public init(
        ...
        prompt: String = "GlobalProtect needs administrator privileges to start the VPN tunnel.",
        ...
        if let sessionsDirectory {
            ...
            self.sessionsDirectory = base.appendingPathComponent("GlobalProtect/sessions", isDirectory: true)
        }
    ...
    public var wrapperURL: URL {
        sessionsDirectory.deletingLastPathComponent().appendingPathComponent("gp-privileged-wrapper.sh")
    }
