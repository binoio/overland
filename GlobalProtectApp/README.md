...
A native SwiftUI/AppKit client for GlobalProtect VPN that drives the
...
* **Root** comes from the standard macOS authorization dialog by default
  ...
  wrapper (`~/Library/Application Support/GlobalProtect/gp-privileged-wrapper.sh`)
...
```
GlobalProtectApp/
├── Package.swift                 GlobalProtectCore (portable) + GlobalProtect (macOS app)
├── Sources/GlobalProtectCore/    models, GpclientCommandBuilder, GpclientOutputParser,
│                                 ProcessRunner, GlobalProtectBridgeService, MockBridgeService,
│                                 PrivilegedProcessRunner (admin dialog), BinaryLocator,
│                                 SudoAskpass, TunInterfaceInspector, …
├── Sources/GlobalProtect/        SwiftUI views, VpnViewModel, AppDelegate, Keychain
├── Tests/GlobalProtectCoreTests/ runs on macOS and Linux (Docker) — 70+ tests
├── Tests/GlobalProtectTests/     view model tests (macOS)
...
zsh GlobalProtectApp/Scripts/build_gpclient.sh

# 2. Run the app from source (finds target/release/gpclient automatically)
zsh GlobalProtectApp/Scripts/run.sh

# 3. Or assemble a self-contained dist/GlobalProtect.app
#    (embeds gpclient, gpauth, their Homebrew dylibs, and the vendored vpnc-script)
zsh GlobalProtectApp/Scripts/bundle.sh
open dist/GlobalProtect.app
```
...
```zsh
zsh GlobalProtectApp/Scripts/test.sh            # swift test on this Mac
zsh GlobalProtectApp/Scripts/test.sh --docker   # GlobalProtectCore in a Linux container
zsh GlobalProtectApp/Scripts/test.sh --all      # both + the gpapi Rust tests the app depends on
```
...
```zsh
GP_SIGN_IDENTITY="Developer ID Application: Your Name (TEAMID)" \
  zsh GlobalProtectApp/Scripts/notarize.sh
```

Signs the embedded dylibs and gpclient, then the app with hardened runtime and
`Support/GlobalProtect.entitlements`, submits to Apple with `notarytool`
(keychain profile `globalprotect-notary`), staples, and writes
`dist/GlobalProtect-<version>.zip`.
