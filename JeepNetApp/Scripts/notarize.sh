#!/bin/zsh
#
# notarize.sh: Sign dist/JeepNet.app with a Developer ID certificate,
# notarize it with Apple, staple the ticket, and produce a distributable zip.
#
# One-time setup:
#   xcrun notarytool store-credentials jeepnet-notary \
#       --key AuthKey_XXXX.p8 --key-id XXXX --issuer <issuer-uuid>
#
# Usage: zsh JeepNetApp/Scripts/notarize.sh
#   JEEPNET_SIGN_IDENTITY="Developer ID Application: Name (TEAMID)"  (required)
#   JEEPNET_NOTARY_PROFILE=jeepnet-notary                       (default)
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
APP_DIR="${REPO_ROOT}/JeepNetApp"
APP="${REPO_ROOT}/dist/JeepNet.app"
ENTITLEMENTS="${APP_DIR}/Support/JeepNet.entitlements"
IDENTITY="${JEEPNET_SIGN_IDENTITY:-}"
NOTARY_PROFILE="${JEEPNET_NOTARY_PROFILE:-jeepnet-notary}"

[[ -d "$APP" ]] || { echo "error: ${APP} missing; run Scripts/bundle.sh first" >&2; exit 1; }
[[ -n "$IDENTITY" ]] || { echo "error: set JEEPNET_SIGN_IDENTITY to your Developer ID Application identity" >&2; exit 1; }
...
ZIP="${REPO_ROOT}/dist/JeepNet-${VERSION}.zip"
