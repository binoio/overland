...
# dist/JeepNet.app, embedding gpclient, its Homebrew dylibs, and
# OpenConnect's vpnc-script so the bundle is self-contained.
#
# Usage: zsh JeepNetApp/Scripts/bundle.sh [--skip-gpclient]   (--skip-gpclient: do not embed gpclient; rely on Homebrew or a custom path)
...
APP_BUNDLE="${DIST_DIR}/JeepNet.app"
...
echo "==> Building JeepNet (release)"
xcrun swift build -c release --package-path "$APP_DIR" --product JeepNet
...
cp "${BIN_PATH}/JeepNet" "${MACOS_DIR}/JeepNet"
chmod +x "${MACOS_DIR}/JeepNet"

# SwiftPM resource bundle (icon etc.) plus a flat copy of the icon for CFBundleIconFile.
if [[ -d "${BIN_PATH}/JeepNet_JeepNet.bundle" ]]; then
    ditto "${BIN_PATH}/JeepNet_JeepNet.bundle" "${RESOURCES_DIR}/JeepNet_JeepNet.bundle"
fi
cp "${APP_DIR}/Sources/JeepNet/Resources/AppIcon.icns" "${RESOURCES_DIR}/AppIcon.icns"
...
codesign --force --deep --sign - --entitlements "${APP_DIR}/Support/JeepNet.entitlements" "$APP_BUNDLE" >/dev/null
