...
JeepNet is a native SwiftUI/AppKit macOS client for GlobalProtect VPN portals. It drives the
...
* **Root** comes from the standard macOS authorization dialog by default
  ...
  wrapper (`~/Library/Application Support/JeepNet/jeepnet-privileged-wrapper.sh`)
...
```
JeepNetApp/
├── Package.swift                 JeepNetCore (portable) + JeepNet (macOS app)
├── Sources/JeepNetCore/    models, GpclientCommandBuilder, GpclientOutputParser,
...
├── Sources/JeepNet/        SwiftUI views, VpnViewModel, AppDelegate, Keychain
├── Tests/JeepNetCoreTests/ runs on macOS and Linux (Docker) — 70+ tests
├── Tests/JeepNetTests/     view model tests (macOS)
...
zsh JeepNetApp/Scripts/build_gpclient.sh

# 2. Run the app from source (finds target/release/gpclient automatically)
zsh JeepNetApp/Scripts/run.sh

# 3. Or assemble a self-contained dist/JeepNet.app
#    (embeds gpclient, gpauth, their Homebrew dylibs, and the vendored vpnc-script)
zsh JeepNetApp/Scripts/bundle.sh
open dist/JeepNet.app
```
...
```zsh
zsh JeepNetApp/Scripts/test.sh            # swift test on this Mac
zsh JeepNetApp/Scripts/test.sh --docker   # JeepNetCore in a Linux container
zsh JeepNetApp/Scripts/test.sh --all      # both + the gpapi Rust tests the app depends on
```
...
```zsh
JEEPNET_SIGN_IDENTITY="Developer ID Application: Your Name (TEAMID)" \
  zsh JeepNetApp/Scripts/notarize.sh
```

Signs the embedded dylibs and gpclient, then the app with hardened runtime and
`Support/JeepNet.entitlements`, submits to Apple with `notarytool`
(keychain profile `jeepnet-notary`), staples, and writes
`dist/JeepNet-<version>.zip`.
