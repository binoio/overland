...
public enum AppLocationCheck {
    ...
    public static func promptToMoveOutOfDownloadsIfNeeded() -> Bool {
        ...
        alert.messageText = "Move JeepNet to Applications?"
        alert.informativeText = "Running JeepNet from /Applications ensures reliable background helper integration and system permissions."
        alert.addButton(withTitle: "Move to Applications")
        alert.addButton(withTitle: "Continue Running Here")

        let response = alert.runModal()
        if response == .alertFirstButtonReturn {
            let dest = URL(fileURLWithPath: "/Applications/JeepNet.app")
