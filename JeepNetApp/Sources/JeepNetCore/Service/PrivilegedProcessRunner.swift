...
public actor PrivilegedProcessRunner: ProcessRunning {
    ...
    #!/bin/bash
    # JeepNet: root-side launcher.
    # Usage: jeepnet-privileged-wrapper.sh <session-dir> <executable> [args…]
    #
    ...
    public init(
        ...
        prompt: String = "JeepNet needs administrator privileges to start the VPN tunnel.",
        ...
        if let sessionsDirectory {
            ...
            self.sessionsDirectory = base.appendingPathComponent("JeepNet/sessions", isDirectory: true)
        }
    ...
    public var wrapperURL: URL {
        sessionsDirectory.deletingLastPathComponent().appendingPathComponent("jeepnet-privileged-wrapper.sh")
    }
