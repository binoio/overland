...
public struct SudoAskpass: Sendable {
    ...
    #!/bin/sh
    # JeepNet: SUDO_ASKPASS helper.
    # Presents a native password dialog and prints the password for sudo.
    exec /usr/bin/osascript <<'APPLESCRIPT'
    set promptText to "JeepNet needs administrator privileges to start the VPN tunnel (OpenConnect must create the utun device).\\n\\nEnter your macOS password:"
    set dialogResult to display dialog promptText default answer "" with hidden answer with title "JeepNet" with icon caution buttons {"Cancel", "OK"} default button "OK"
    ...
    public init(directory: URL? = nil) {
        if let directory {
            ...
            self.directory = base.appendingPathComponent("JeepNet", isDirectory: true)
        }
    }

    public var scriptURL: URL {
        directory.appendingPathComponent("jeepnet-askpass.sh")
    }
